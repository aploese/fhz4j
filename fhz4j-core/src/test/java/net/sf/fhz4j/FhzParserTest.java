...
public class FhzParserTest implements FhzDataListener {
    ...
    public void testSomeMethod() {
        ...
        assertEquals("housecode: 2701, command: valve, description: same pos absolute, value: 0.0% signal strength: 0.0", fhtMessage.toString());

        decode("T4D5400A600\r\n");
        assertNotNull(fhtMessage);
        assertEquals("housecode: 7784, command: valve, description: same pos absolute, value: 0.0% signal strength: 0.0", fhtMessage.toString());
        decode("T435E002626\r\n");
        assertNotNull(fhtMessage);
        assertEquals("housecode: 6794, command: valve, description: changed pos absolute, value: 14.9% signal strength: 0.0", fhtMessage.toString());
        decode("T212700262C\r\n");
        assertNotNull(fhtMessage);
        assertEquals("housecode: 3339, command: valve, description: changed pos absolute, value: 17.3% signal strength: 0.0", fhtMessage.toString());
        decode("T1B0100A600\r\n");
        assertNotNull(fhtMessage);
        assertEquals("housecode: 2701, command: valve, description: same pos absolute, value: 0.0% signal strength: 0.0", fhtMessage.toString());
        decode("T4D5400A600\r\n");
    ...
    public void decode_FHT_25_1_Degree_Centigrade() {
        ...
        assertNotNull(fhtMessage);
        assertEquals("housecode: 9752, command: measured low, description: from FHT-B data register, value: 25.1°C signal strength: -76.0", fhtMessage.toString());
    }
    ...
    public void decode_FHT_25_9_Degree_Centigrade() {
        ...
        assertNotNull(fhtMessage);
        assertEquals("housecode: 5510, command: measured low, description: from FHT-B data register, value: 0.4°C signal strength: -71.0", fhtMessage.toString());
        decode("T370A43690106\r\n");
        assertNotNull(temp);
        assertEquals("housecode: 5510, measured temperature (combined): 25.9°C signal strength: -71.0 and -71.0", temp.toString());
    }
